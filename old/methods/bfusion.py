...
vpath  = os.path.join(model_path, '/%s%d/final-V.dat');
...
def get_weights(trscores, uids, trvids, i):
    rf = RankingFusion(trscores, uids, trvids);
    trdata = list();
    for line in open(trpath%i):
        ...
        for j in range(1, len(terms)):
            ...
            if like == 1:
                trdata.append((uid, vid));
    rf.train(trdata, epochs = 5);
    weights = np.array(rf.W.eval());
    return weights;
...
def main():
    ...
    for i in range(start, end):
        ...
        for mode in scenario:
            ...
            bfusion = do_fusion(weights1, tescores1);
            tresults, tcount = doEvaluate(bfusion, tepath%(i, mode), uids, tevids, invids);
            for j in range(interval):
                tresults[j] = tresults[j] * 1.0 / tcount;
            if 'bfusion1' not in results[mode]:
                results[mode]['bfusion1'] = np.array(tresults);
            else:
                for j in range(interval):
                    results[mode]['bfusion1'][j] += tresults[j];
            bfusion = do_fusion(weights2, tescores2); 
            tresults, tcount = doEvaluate(bfusion, tepath%(i, mode), uids, tevids, invids);
            for j in range(interval):
                tresults[j] = tresults[j] * 1.0 / tcount;
            if 'bfusion2' not in results[mode]:
                results[mode]['bfusion2'] = np.array(tresults);
            else:
                for j in range(interval):
                    results[mode]['bfusion2'][j] += tresults[j];
